...
4. 对真实观测无条件把该结果保存在 `raw_palm_local_joints_m`；短缺口插值记录的 raw 字段保持为空；
...
`cross_view_weighting: equal` 和主输出字段。本节实验使用
`--zero-shot-primary-output raw`、bone calibration 为 0、temporal smoothing 为 0。后续在
`bad_failure` 序列完成抖动验证后，离线视频主 pipeline 已改为默认使用 5 帧 Gaussian 主输出，
同时继续单独保留 raw。当前流水线另有最多 2 帧的有界缺口插值；复现本节历史实验时应同时
关闭平滑并把插值设为 0。
...
本节实验的 Gaussian 版本需要向后看十帧，只是 offline option，不能冒充 causal deployment
结果；当前视频 pipeline 的默认窗口已缩短为前后各 2 帧。
...
默认计算这个可选字段；实时处理可把它选为主输出，离线视频主输出默认使用 Gaussian。
