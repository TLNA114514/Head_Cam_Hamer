...
此前通过完整序列验收的 `realtime` 单 prompt 速度配置是：
...
这些 FPS 属于仅使用 `hand` 的速度基线，不是当前左右手准确性默认。`video/7.17` 暴露出该模式会把
同一只手的嵌套掩码按画面横坐标拆成左右手。当前默认改为 `bare` 左右手语义 prompt、0.9 掩码包含
去重、轨迹标签滞回和跨相机全局双假设选择。在该 384 帧单右手样例上，最终输出由 758 只融合手
修正为 **384/384 帧各一只 Right**；共去掉 11 个嵌套候选，无空帧。四相机真实全链路为
6.080 冷启动端到端 FPS、7.508 稳态 FPS（不含已复用的去畸变阶段）。这是手数/左右手正确性回归，
不等价于有 GT 的 3D 关节精度评估。
...
多路相机图像
  ...
  → 在 palm-local 坐标中用整手 medoid 融合多相机结果
  → 在骨向量空间用 One Euro 做动作保真的因果平滑
  → 输出 filtered 主结果 + raw 复核结果
```
...
| 目标 | 推荐配置 |
| --- | --- |
| 需要已验证的冷启动 ≥10 FPS | `--sam3-prompt-preset realtime` + C0,C2,C3；必须另验左右手 |
| 模型常驻、希望保留四相机 | 2 SAM3 GPU + CPU MobRecon FP32，或 1 SAM3 + GPU MobRecon FP32 |
| 精度优先研究 | `bare` prompt、四路 per-view、FP32、raw + One Euro 两份输出 |
| 保持 HaMeR 接口 | `quality` profile + per-sequence 加载 + job packing |
| 离线最快但允许精度变化 | 单独验证 `balanced` / `aggressive`，不要直接用于正式输出 |

## 当前推荐实时命令

旧实验中的 `beta=0.05` 在米制关节上会产生明显动作滞后，适合压低静态误差，但不能代表握拳等动态
动作的保真度。当前低延迟默认按 `Hz/(m/s)` 使用 `beta=100.0`、`min_alpha=0.4`，不读取未来帧，
并保留 raw 字段用于复核。
...
下表是方法概览索引；完整 50 项实验和四相机全部 15 种组合见技术附录 O01--O50。
...
| 左右手 | 语义 prompt + 包含去重 + 轨迹滞回 | 6.080 E2E / 7.508 steady FPS | — | 384/384 单 Right | 精度默认 | O50 |
