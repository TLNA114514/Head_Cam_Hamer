...
MobRecon 现在默认使用 `C0,C1,C2,C3` 四相机，作为准确性优先配置：更多视角能提高遮挡情况下的
覆盖率，也给 `robust-medoid` 提供更多完整手型候选。这组设置对应当前已验证的
“双 SAM3 GPU producer + CPU MobRecon”调度。旧的 `C0,C2,C3` 三相机组合仅作为吞吐优化选项；
只有确认 C1 质量较差或四相机速度不满足要求时，才建议通过 `--cameras C0,C2,C3` 显式启用。
...
主命令显式写出平滑参数，便于复现实验。常用调整方向如下：

| 目标 | `min-cutoff` | `beta` | `min-alpha` | 结果倾向 |
|---|---:|---:|---:|---|
| 当前推荐 | `0.25` | `100` | `0.4` | 低延迟与稳定性的平衡 |
| 更平滑 | `0.20` | `80` | `0.3` | 静止更稳，但动作响应稍慢 |
| 更低延迟 | `0.25` | `150` | `0.5` | 更快跟随动作，但抖动可能增加 |

`--one-euro-bone-space` 建议保持开启；它在父子骨向量空间滤波，并单独稳定骨长。
`--frame-rate` 应填写数据的实际帧率。以上滤波均为因果处理，不读取未来帧。
...
该数据集命令沿用上方“当前推荐”的因果平滑配置；需要固定实验参数时，可直接追加主 MobRecon
示例中的平滑参数块。
...
MobRecon 会优先选择同时具备源码和 `pretrained/mobrecon_densestack.pt` 的 HandMesh 目录。
为兼容旧安装，如果 `external/HandMesh` 缺少权重而 `third_party/HandMesh` 完整，则会自动使用后者。
正式部署仍推荐使用 `external/HandMesh`，或通过 `MOBRECON_ROOT` 明确指定唯一目录。
